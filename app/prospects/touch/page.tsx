...
export default function TouchProspectsPage() {
  ...
  const fetchProspects = () => {
    setLoading(true);
    fetch("/api/admin/touch/prospects")
      .then((r) => r.json())
      .then((data) => {
  ...
  const handleCollect = async () => {
    ...
    try {
      ...
      for (const prefecture of selectedPrefs) {
        const res = await fetch("/api/agent/touch/collect", {
          method: "POST",
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <h1 className="text-2xl font-bold text-gray-800 mb-6">整骨院プロスペクト管理</h1>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    {["院名", "都道府県", "ステータス", "メール", "評価", "送信日", "登録日"].map((h) => (
                      <th
                        key={h}
